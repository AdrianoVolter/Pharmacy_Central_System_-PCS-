...
module.exports = {
  ...
  async criarDeposito(req, res) {
    try {
      ...
      const depositoExiste = await Depositos.findOne({
        where: {
          cnpj: cnpj,
      ...
      if (depositoExiste) {
        const depositoUsuarioExiste = await DepositosUsuarios.findOne({
          where: {
        ...
        if (depositoUsuarioExiste) {
          return res
            .status(409)
            .send({ error: "O deposito Ja existe!", CNPJ: cnpj, razao_social });
        } else {
      ...
      if (razao_social.length < 3 || nome_fantasia.length < 3) {
        return res
          ...
          .send({
            error:
              "Razão Social e nome Fantasia devem ter no mínimo 3 caracteres!",
          });
      }
      const deposito = await Depositos.create({
        razao_social,
        cnpj,
        nome_fantasia,
        email,
      ...
      if (!deposito) {
        return res
          .status(400)
          .send({ error: "Não foi possivel criar o depósito!" });
      } else {
        return res
          .status(201)
          .send({
            Identificador: deposito.id,
            razao_social: deposito.razao_social,
            Usuario: usuario,
            deposito,
          });
      }
  ...
  async atualizarDeposito(req, res) {
    try {
      ...
      if (nome_fantasia.length < 3) {
        return res
          .status(400)
          .send({ error: "O nome fantasia deve ter no mínimo 3 caracteres!" });
      }
      const depositoUsuario = await DepositosUsuarios.findOne({
        where: { id_depositos: id, id_usuarios: req.usuario.id },
      });
      if (!depositoUsuario) {
        return res
          .status(403)
  ...
  async atualizarStatusDeposito(req, res) {
    try {
      ...
      if (!deposito) {
        ...
        await Depositos.update({ status }, { where: { id: id } });
        return res
          .status(204)
          .send({
            message: "Status do depósito atualizado com sucesso!",
            deposito: { id, status },
          });
      }
    } catch (error) {
    ...
    try {
      ...
      if (req.query.status === undefined) {
        ...
        if (!depositos) {
          ...
          return res
            .status(200)
            .send({ message: "Depósitos encontrados!", depositos });
        }
      }
  ...
  async listarDepositoId(req, res) {
    try {
      ...
      if (!deposito) {
        ...
        return res
          .status(200)
          .send({ message: "Depósito encontrado!", deposito });
      }
    } catch (error) {
      return res.status(400).send({ error: error.message });
    }
    ...
    try {
      ...
      if (!deposito) {
        ...
        if (deposito.status === "Ativo") {
          return res
            .status(400)
            .send({ error: "Não é possível excluir um depósito ativo!" });
        }
        ...
        return res
          .status(204)
          .send({ message: "Depósito excluído com sucesso!" });
      }
    } catch (error) {
      return res.status(400).send({ error: error.message });
    }
